...
| **L8** | **`env` divergente**: `production` na API, `prod-simulated` na Lambda | Tag `env:prod-simulated` nas métricas da função | Uma variável de template `$env` não cobre os dois. Monitores da Lambda escopam por `functionname` | PR aberto no repositório da Lambda |
...
| --- | --- | --- |
| `env`: `production` (API) contra `prod-simulated` (Lambda) | **`production`** é a fonte de verdade | Consultas da API usam `env:production`; consultas da Lambda **não escopam por `env`**, e sim por `functionname`. Um PR aberto no repositório da Lambda unifica o valor, e não é pré-requisito de nada aqui |
