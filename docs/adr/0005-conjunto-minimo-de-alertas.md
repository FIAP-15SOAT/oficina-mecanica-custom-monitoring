...
| # | Sintoma | Prioridade |
| --- | --- | --- |
| M1 | Rota pública indisponível (verificação externa) | P1 |
| M2 | Erros 5xx da API acima do limite, por rota | P2 |
| M3 | Latência p95 acima do alvo | P3 |
| M4 | Falhas no fluxo de ordens de serviço | P2 |
| M5 | Memória do pod próxima do limite | P2 |
| M6 | Reinícios de contêiner | P3 |
| M7 | Erros de execução da Lambda | P2 |
| M8 | Falha de envio de e-mail | P4 |
| M9 | Dependência degradada | P3 |
...
### Contagem, não taxa, em M2, M4 e M8
...
### A redundância entre M2 e M4 é assumida

M2 e M4 disparam juntos numa indisponibilidade total, e isso **não é um defeito a corrigir**. São leituras
...
| --- | --- |
| **Monitor composto** (`M2 AND NOT M4`) | Cerimônia desproporcional: acrescenta um recurso, uma indireção e uma classe nova de defeito para evitar um e-mail duplicado ocasional |
| **Um único monitor agrupado por rota** | Perderia a nomeação do impacto de negócio, que é o requisito de M4 |
| **Excluir as rotas de negócio do escopo de M2** | Uma falha geral deixaria de aparecer no alerta de API, que é justamente o alerta que se olha primeiro |
...
| **CPU alta** | Com autoescalonamento mirando 70 % de CPU, "CPU alta" é o mecanismo **funcionando**. Alertar sobre isso é alertar sobre o sucesso | Estrangulamento sustentado correlacionado com M3 |
| **Estrangulamento de CPU** | É o sinal acionável que "CPU alta" não é — mas, sem evidência de que ele preceda degradação percebida nesta aplicação, seria um alerta em busca de um problema. Fica no dashboard de Kubernetes | M3 disparar com estrangulamento como causa recorrente |
...
| **Latência de banco em monitor próprio** | Sem latência de API alta, latência de banco alta não é incidente. Com ela, M3 já disparou e o dashboard de API mostra a causa no grupo Banco | — |
...
| **Timeouts e falta de memória da Lambda, em monitores próprios** | Ambos incrementam `aws.lambda.enhanced.errors`, que M7 já observa. Seriam três e-mails para o mesmo evento | — |
...
**Os limiares de M3 são valores de percepção de usuário, não percentis observados**, e estão declarados como
...
- **M8 e M9 não podem ser verificados de ponta a ponta** sem provocar o evento na origem: `mail.send.failed` e
  `health.degraded` têm zero ocorrências no índice. Estão marcados assim na documentação, e não como
  "funcionando".
- **M7 avalia em `No Data`, não em `OK`**, na operação normal, porque `aws.lambda.enhanced.errors` só passa a
